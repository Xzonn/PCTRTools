...
{
  private readonly List<List<List<ushort>>> Keys = new()
  {
    new List<List<ushort>>(),
    new List<List<ushort>>()
  };
  private readonly List<List<List<ushort>>> Unknowns = new()
  {
    new List<List<ushort>>(),
    new List<List<ushort>>()
  };
  private readonly List<List<byte>> Unknowns2 = new()
  {
    new List<byte>(),
    new List<byte>()
  };
  private readonly List<List<List<ushort>>> OriginalLengths = new()
  {
    new List<List<ushort>>(),
    new List<List<ushort>>()
  };
  ...
  public TextGen5(NARC files)
  {
    ...
    foreach (byte[] bytes in files.Files)
    {
      ...
      for (int i = 0; i < TextList.Count; i++)
      {
        ...
        List<ushort> unknowns = new();
        byte unk2 = 0;
        List<ushort> characterCount = new();
        if (i < numSections)
        {
          br.BaseStream.Position = sectionOffset[i];
          sizeSections[i] = br.ReadUInt32();

          List<uint> tableOffsets = new();
          List<List<ushort>> encText = new();
  ...
  public void Save(ref NARC files)
  {
    for (int i = 0; i < files.Files.Count; i++)
    {
      ...
      BinaryReader br = new(new MemoryStream(files.Files[i]));
  ...
  private byte[] MakeSection(List<string> s, int numSection, int fileCount, int numEntries)
  {
    ...
    MemoryStream mss = new();
    BinaryWriter bws = new(mss);
    bws.Write(size);
    for (int k = 0; k < numEntries; k++)
    {
